...
void get_filesystem(char *compression, int clientfd) {
    char *exclude = "--exclude=sys --exclude=proc --exclude=dev --exclude=mnt --exclude=var/run/docker --exclude=var/lib/docker --exclude=var/lib/lxcfs --exclude=var/lib/vm2docker/filesystem.tar.gz"; //TODO: Updated by manny
    char *target = "/";
